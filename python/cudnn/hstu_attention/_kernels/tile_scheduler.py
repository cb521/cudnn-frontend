...
class QMajorBwdScheduler:
    """Assign one CTA to a query-head pair and visit its K tiles in order.

    The regular backward scheduler assigns one CTA to every K tile.  When a
    sequence contains exactly one query, that creates many CTAs which all
    reload the same Q/dO tile and contribute to the same dQ row.  This
    scheduler keeps those K tiles on one persistent CTA while preserving the
    work-tile interface consumed by the fused backward pipeline.
    """
