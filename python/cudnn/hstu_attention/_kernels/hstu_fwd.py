    ...
    def _silu_step_q1_m64(
        ...
        if const_expr(self.q1_m64_16dp_silu):
            ...
            for chunk in cutlass.range_constexpr(self.kBlockN // 32):
                ...
                for i in cutlass.range_constexpr(16):
                    ...
                    scores_16dp[score_idx] = values_16dp[i] if const_expr(self.q1_m64_tail_branch) or col < valid_cols else Float32(0.0)
            cute.arch.fence_view_async_tmem_load()
            if const_expr(self.q1_m64_tail_branch):
                if valid_cols < self.kBlockN:
                    for chunk in cutlass.range_constexpr(self.kBlockN // 32):
                        for i in cutlass.range_constexpr(16):
                            scores_16dp[chunk * 16 + i] = scores_16dp[chunk * 16 + i] if chunk * 32 + i * 2 + lane_parity < valid_cols else Float32(0.0)
        ...
        if const_expr(self.q1_m64_silu_warps == 0):
            ...
            if warp_idx < self.q1_m64_silu_warps:
                ...
                for i in cutlass.range_constexpr(self.kBlockN):
                    active_scores[i] = active_scores[i] if i < valid_cols else Float32(0.0)
