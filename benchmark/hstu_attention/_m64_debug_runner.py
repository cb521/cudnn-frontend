...
outputs = {}
for implementation in ("tc", "tc-m64"):
...
for row, length in enumerate(k_lengths):
    ...
    m64_error = (outputs["tc-m64"][row] - reference[row]).abs().max().item()
    cross_error = (outputs["tc-m64"][row] - outputs["tc"][row]).abs().max().item()
    print(f"LEN {length:4d} TC {tc_error:.8e} M64 {m64_error:.8e} CROSS {cross_error:.8e}", flush=True)
    if length in (128, 2048):
        print("  REF", reference[row, 0, :8].tolist(), flush=True)
        print("  TC ", outputs["tc"][row, 0, :8].tolist(), flush=True)
        print("  M64", outputs["tc-m64"][row, 0, :8].tolist(), flush=True)
